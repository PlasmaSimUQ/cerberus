        ...
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                for (int s = 0; s < reco.stencil_length; ++s) {
                    ...
                    stencil[s] =
                      src4(i + stencil_index[0], j + stencil_index[1], k + stencil_index[2], index);
                }
 ...
 */
void MFP::getPlotData(MultiFab& plot_data, std::vector<std::string>& plot_names)
{
    ...
    for (int fi = 0; fi < nf; ++fi) { plot_names[nv + fi] = plot_functions[fi].first; }
    ...
    std::map<std::string, Real> dat = {{"Larmor", Larmor},
                                       {"Debye", Debye},
                                       {"c", lightspeed},
    ...
    for (MFIter mfi(cost); mfi.isValid(); ++mfi) {
        ...
        for (const auto& pair : plot_variables) { dat_arrays[pair.first].resize(orig_box, 1); }

        // cost
        const bool get_cost = plot_variables.find("cost") != plot_variables.end();
        if (get_cost) { dat_arrays["cost"].copy(cost[mfi]); }
...
void MFP::writePlotFile(const std::string& dir, std::ostream& os, VisMF::How how)
{
    ...
    if (level == 0 && ParallelDescriptor::IOProcessor()) {
        ...
        if (n_data_items == 0) amrex::Error("Must specify at least one valid data item to plot");
        ...
        // Names of variables
        //
        for (int i = 0; i < n_data_items; i++) { os << plot_names[i] << '\n'; }

        os << AMREX_SPACEDIM << '\n';
        os << cur_time << '\n';
        int f_lev = parent->finestLevel();
        os << f_lev << '\n';
        for (int i = 0; i < AMREX_SPACEDIM; i++) os << Geom().ProbLo(i) << ' ';
        os << '\n';
        for (int i = 0; i < AMREX_SPACEDIM; i++) os << Geom().ProbHi(i) << ' ';
        os << '\n';
        ...
        for (int i = 0; i <= f_lev; i++) {
            for (int k = 0; k < AMREX_SPACEDIM; k++) os << parent->Geom(i).CellSize()[k] << ' ';
            os << '\n';
        }
        os << (int)Geom().Coord() << '\n';
        os << "0\n";  // Write bndry data.
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        if (level == parent->finestLevel()) {
            for (int lev = 0; lev <= parent->finestLevel(); ++lev) { os << "1.0e-6\n"; }
        }
#endif
    }
...
void MFP::archive_folder(const std::string& dir)
{
    ...
    for (const auto& rm : to_remove) { FileSystem::RemoveAll(rm); }
...
void MFP::writePlotFilePost(const std::string& dir, std::ostream& os)
{
    ...
    if (level != parent->finestLevel() || !ParallelDescriptor::IOProcessor()) return;
    ...
    if (!InfoFile.good()) { amrex::FileOpenFailed("info"); }
    ...
    if (!InputsFile.good()) { amrex::FileOpenFailed("inputs"); }
...
void MFP::checkPointPost(const std::string& dir, std::ostream& os)
{
    ...
    if (archive_checkpoint) archive_folder(dir);
}

void MFP::write_info(nlohmann::json& js) const
{
    BL_PROFILE("MFP::write_info");
