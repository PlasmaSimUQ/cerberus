...
void Action::get_data(MFP* mfp,
                      Vector<Array<int, 2>>& options,
                      Vector<UpdateData>& update,
                      const Real time) const
{
    BL_PROFILE("Action::get_data");
    ...
    for (const auto& index : options) {
        ...
        if (fill) {
            if (U_status != UpdateData::Status::Filled) {
                ...
                if ((U_status == UpdateData::Status::Inactive) ||
                    (update[data_idx].U.nGrow() < ng)) {
                    update[data_idx].U.define(mfp->boxArray(),
                                              mfp->DistributionMap(),
                                              ns,
                                              ng,
                                              MFInfo(),
            ...
            if ((U_status != UpdateData::Status::Local) &&
                (U_status != UpdateData::Status::Filled)) {
        ...
        if (dU_status == UpdateData::Status::Inactive) {
            update[data_idx].dU.define(mfp->boxArray(),
                                       mfp->DistributionMap(),
                                       ns,
                                       num_grow_eb,
                                       MFInfo(),
        ...
        if ((dU_status == UpdateData::Status::Expired) ||
            (dU_status == UpdateData::Status::Inactive)) {
