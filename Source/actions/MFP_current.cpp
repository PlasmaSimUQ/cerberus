...
Current::Current(const int idx, const sol::table& def)
{
    ...
    if (!state_name.empty()) {
        ...
        case State::StateType::Field:
            ...
            break;
        default: Abort("An invalid state has been defined for the Current source " + name);
        }
    } else {
...
void Current::calc_time_derivative(MFP* mfp,
        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (int d = 0; d < 3; ++d) {
                        field_dU4(i, j, k, +FieldDef::ConsIdx::Dx + d) +=
                          dt * scale_factor * current[d](Q);
                    }
                }
            }
        }
