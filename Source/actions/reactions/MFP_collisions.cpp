...
Collisions::Collisions(const int idx, const sol::table& def)
{
    ...
    for (const auto& key_value_pair : state_names) {
        ...
        default: Abort("An invalid state has been defined for the Collisions source " + name);
        }
    }
...
void Collisions::get_data(MFP* mfp, Vector<UpdateData>& update, const Real time) const
{
    ...
    for (size_t i = 0; i < states.size(); ++i) { options[i] = {states[i]->global_idx, 0}; }

    Action::get_data(mfp, options, update, time);
}

void Collisions::calc_time_derivative(MFP* mfp,
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (int si = 0; si < n_states; ++si) {
                        ...
                        for (size_t l = 0; l < Un.size(); ++l) { Un[l] = U4(i, j, k, l); }
                    ...
                    for (int n = 0; n < n_species; ++n) {
                        ...
                        if (Un[+HydroDef::ConsIdx::Density] >
                            states[info.state_idx]->effective_zero) {
                    ...
                    for (int si = 0; si < n_states; ++si) {
                        for (int ai = 0; ai < alpha[si].size(); ++ai) {
                            for (int ci = 0; ci < +HydroDef::ConsIdx::NUM; ++ci) {
                                accounting[si][ai][ci] =
                                  U[si][ci] *
                                  alpha[si][ai];  // original amount of each conserved quantity
                            }
                        }
                    }
                    ...
                    for (int isp = 0; isp < n_species; ++isp) {
                        ...
                        for (int ci = 0; ci < +HydroDef::ConsIdx::NUM; ++ci) {
                            accounting[info.state_idx][info.alpha_idx][ci] +=
                              dt * info.delta[ci];  // updated amount
                        }
                    }
                    ...
                    for (int si = 0; si < n_states; ++si) {
                        ...
                        for (int ci = 0; ci < +HydroDef::ConsIdx::NUM; ++ci) {
                            // accumulate how much stuff is held by all the sub-components in this
                            // state
                            Real sum = 0.0;
                            for (const auto& acc : accounting[si]) { sum += acc[ci]; }
                        ...
                        for (int ti = 0; ti < states[si]->n_tracers; ++ti) {
                            state_dU4[si](i, j, k, +HydroDef::ConsIdx::NUM + ti) +=
                              accounting[si][ti][+HydroDef::ConsIdx::Density] -
                              state4[si](i, j, k, +HydroDef::ConsIdx::NUM + ti);
                        }
                    }
                }
            }
