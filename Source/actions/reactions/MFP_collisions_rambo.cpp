...
bool CollisionsRambo::registered =
  GetActionFactory().Register(CollisionsRambo::tag, ActionBuilder<CollisionsRambo>);
...
{
    BL_PROFILE("CollisionsRambo::CollisionsRambo");
    ...
    if (!cross_sections) {
        ...
        for (const SpeciesInfo& info : species_info) {
            if (states[info.state_idx]->gas->charge[info.alpha_idx] == 0.0) {
                Abort("Error: coss sections for collisions involving neutral species '" +
                      info.name + "' are required by action '" + name + "'");
            }
        }
    }
...
{
    BL_PROFILE("CollisionsRambo::calc_update");
