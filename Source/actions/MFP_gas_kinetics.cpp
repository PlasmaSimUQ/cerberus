...
bool GasKinetics::registered =
  GetActionFactory().Register(GasKinetics::tag, ActionBuilder<GasKinetics>);
...
{
    BL_PROFILE("GasKinetics::GasKinetics");
    ...
    for (const auto& key_value_pair : state_names) {
        ...
        case State::StateType::Hydro:
            ...
            break;
        default: Abort("An invalid state has been defined for the 'gas' action '" + name + "'");
        }
    }
    ...
    if (gas_model_id < 0)
        Abort("Action '" + name + "' has failed when trying to create a new gas model");
    ...
    for (int i = 0; i < n_species; ++i) {
        ...
        if (!found.first) Abort("Couldn't find sub-component for '" + sname + "'");
    }
    ...
    if (chemistry_update_file.empty())
        Abort("Action '" + name + "' requires 'chemistry_update' to be defined (a lua file)");

    std::string chemistry_update_file2 = def.get_or<std::string>("chemistry_update2", "");

    thermochemical_reactor_id = thermochemical_reactor_new(gas_model_id,
                                                           chemistry_update_file.data(),
                                                           chemistry_update_file2.data());

    if (thermochemical_reactor_id < 0)
        Abort("Action '" + name + "' has failed when trying to create a thermochemical reactor");
...
{
    BL_PROFILE("GasKinetics::get_data");

    Vector<Array<int, 2>> options(states.size());

    for (size_t s = 0; s < num_states; ++s) { options.push_back({states[s]->global_idx, 0}); }

    Action::get_data(mfp, options, update, time);
}

void GasKinetics::calc_time_derivative(MFP* mfp,
                                       Vector<UpdateData>& update,
                                       const Real time,
                                       const Real dt)
{
    BL_PROFILE("GasKinetics::calc_time_derivative");
    ...
    for (int i = 0; i < +HydroDef::ConsIdx::NUM; ++i) { s_conserved[i].resize(num_states); }
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (int si = 0; si < num_states; ++si) {
                        ...
                        for (size_t l = 0; l < UU.size(); ++l) { UU[l] = UU4(i, j, k, l); }
                    ...
                    const Real int_nrg_sum =
                      nrg_sum -
                      0.5 * (mx_sum * mx_sum + my_sum * my_sum + mz_sum * mz_sum) / rho_sum;
                    ...
                    for (int isp = 0; isp < n_species; ++isp) {
                        ...
                        const int aid = species_info[isp].alpha_idx;
                        massf[isp] =
                          alpha[sid][aid] * U_old[sid][+HydroDef::ConsIdx::Density] / rho_sum;
                    }

                    // calculate the mass weighted temperature
                    Real guess_T = 0.0;
                    for (int si = 0; si < num_states; ++si) {
                        guess_T += Q[si][+HydroDef::PrimIdx::Temp] *
                                   s_conserved[+HydroDef::ConsIdx::Density][si] / rho_sum;
                    }
                    ...
                    gas_state_set_scalar_field(
                      gas_model_id,
                      "T",
                      guess_T * MFP::T_ref);  // need an initial guess at the temperature
                    ...
                    thermochemical_reactor_gas_state_update(thermochemical_reactor_id,
                                                            gas_state_id,
                                                            dt * MFP::t_ref,
                                                            &dt_suggest);
                    ...
                    for (int si = 0; si < num_states; ++si) {
                        for (int ai = 0; ai < alpha[si].size(); ++ai) {
                            ...
                            accounting[si][ai][ci] =
                              U_old[si][ci] * alpha[si][ai];  // original amount
                        }
                    }

                    // get the updated amounts of stuff for all sub-components that have been
                    // updated by the reactor
                    // 1. get the change in mass for each species
                    // 2. assume constant temperature and velocity for each grouping and recalculate
                    // momentum and energy
                    ...
                    for (int isp = 0; isp < n_species; ++isp) {
                        ...
                        const int ci = +HydroDef::ConsIdx::Density;
                        accounting[si.state_idx][si.alpha_idx][ci] =
                          massf[isp] * cons_sum[ci];  // updated amount
                    }
                    ...
                    for (int si = 0; si < num_states; ++si) {
                        ...
                        for (const auto& acc : accounting[si]) { total_density += acc[ci]; }
                        ...
                        // note that we can simply use the density of the sub-component which we
                        // have already calculated
                        for (int ti = 0; ti < states[si]->n_tracers; ++ti) {
                            dU4[si](i, j, k, +HydroDef::ConsIdx::NUM + ti) +=
                              accounting[si][ti][ci] -
                              U4[si](i, j, k, +HydroDef::ConsIdx::NUM + ti);

                            // update the alpha values in the primitives vector
                            Q[si][+HydroDef::PrimIdx::NUM + ti] =
                              accounting[si][ti][ci] / total_density;
                        }
