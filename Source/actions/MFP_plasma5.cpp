...
Plasma5::Plasma5(const int idx, const sol::table& def)
{
    ...
    if (solver_name == "explicit") {
        ...
        Abort("Plasma5 source needs to know what type of solver to use, options are "
              "['explicit','implicit']");
    }
    ...
    for (const auto& key_value_pair : state_names) {
        ...
        default: Abort("An invalid state has been defined for the Plasma5 source " + name);
        }
    }
...
void Plasma5::calc_time_derivative(MFP* mfp,
                                   Vector<UpdateData>& update,
                                   const Real time,
                                   const Real dt)
{
    BL_PROFILE("Plasma5::calc_time_derivative");

    switch (solver) {
    case TimeIntegrator::ForwardsEuler: explicit_solve(mfp, update, time, dt); break;
    case TimeIntegrator::BackwardsEuler: implicit_solve(mfp, update, time, dt); break;
        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    field_dU4(i, j, k, +FieldDef::ConsIdx::phi) +=
                      -field4(i, j, k, +FieldDef::ConsIdx::phi) * f3 + f2 * charge_density;

                    // source for B divergence correction
                    field_dU4(i, j, k, +FieldDef::ConsIdx::psi) +=
                      -field4(i, j, k, +FieldDef::ConsIdx::psi) * f3;
                }
            }
        }
        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    // note that the linear system has solved for the updated field but we want the
                    // delta value hence we calculate delta = new - old
                    field_dU4(i, j, k, +FieldDef::ConsIdx::Dx) +=
                      c(n_species * 3 + 0) * ep - field4(i, j, k, +FieldDef::ConsIdx::Dx);
                    field_dU4(i, j, k, +FieldDef::ConsIdx::Dy) +=
                      c(n_species * 3 + 1) * ep - field4(i, j, k, +FieldDef::ConsIdx::Dy);
                    field_dU4(i, j, k, +FieldDef::ConsIdx::Dz) +=
                      c(n_species * 3 + 2) * ep - field4(i, j, k, +FieldDef::ConsIdx::Dz);
                    ...
                    for (int n = 0; n < n_species; ++n) {
                        ...
                        species_dU4[n](i, j, k, +HydroDef::ConsIdx::Eden) +=
                          dt * (R[n] * lightspeed / Larmor) * (Ex * mx + Ey * my + Ez * mz);
                    }
...
{
    BL_PROFILE("Plasma5::get_allowed_time_step");
    ...
    for (int d = 1; d < AMREX_SPACEDIM; ++d) { dt = std::min(dt, dx[d] / max_speed); }
    ...
    for (MFIter mfi(cost); mfi.isValid(); ++mfi) {
        ...
        for (int n = 0; n < n_species; ++n) { species4[n] = species_data[n]->array(mfi); }
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (size_t n = 0; n < n_species; ++n) {
                        ...
                        if (omega_p > 0) { dt = std::min(dt, 1 / omega_p); }

                        const Real omega_c = 10 * (std::abs(r) * B) / (Larmor * 2 * PI);

                        if (omega_c > 0) { dt = std::min(dt, 1 / omega_c); }
                    }
                }
            }
        }
