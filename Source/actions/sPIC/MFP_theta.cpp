...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE Array1D<Real,0,2> curl_cdiff_1(Array4<Real const> const& a ,int i , int j ,int k, const double* ics){
#if AMREX_SPACEDIM == 3
    return {
    ...
    return {
        (a(i,j+1,k,Z)-a(i,j-1,k,Z))*ics[Y]*0.5,
                -( a(i+1,j,k,Z)-a(i-1,j,k,Z))*ics[X]*0.5,
                ((a(i+1,j,k,Y)-a(i-1,j,k,Y))*ics[X]*0.5-( a(i,j+1,k,X)-a(i,j-1,k,X))*ics[Y]*0.5)
    };
#else
    return {0,-(a(i+1,j,k,Z)-a(i-1,j,k,Z))*ics[X]*0.5,(a(i+1,j,k,Y)-a(i-1,j,k,Y))*ics[X]*0.5
    };
#endif
}
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE Array1D<Real,0,2> curl_fdiff_1(Array4<Real const> const& a ,int i , int j ,int k, const double* ics){
#if AMREX_SPACEDIM == 3
    return {
...
#elif AMREX_SPACEDIM == 2
    return {
        (a(i,j+1,k,Z)-a(i,j,k,Z))*ics[Y],
                -( a(i+1,j,k,Z)-a(i,j,k,Z))*ics[X],
                ((a(i+1,j,k,Y)-a(i,j,k,Y))*ics[X]-( a(i,j+1,k,X)-a(i,j,k,X))*ics[Y])
    };
#else
    return {0,-( a(i+1,j,k,Z)-a(i,j,k,Z))*ics[X],(a(i+1,j,k,Y)-a(i,j,k,Y))*ics[X]};
#endif
}

// backward difference, first order
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE Array1D<Real,0,2> curl_bdiff_1(Array4<Real const> const& a ,int i , int j ,int k, const double* ics){
...
#elif AMREX_SPACEDIM == 2
    return {
        (a(i,j,k,Z)-a(i,j-1,k,Z))*ics[Y],
                -( a(i,j,k,Z)-a(i-1,j,k,Z))*ics[X],
                (a(i,j,k,Y)-a(i-1,j,k,Y))*ics[X]-( a(i,j,k,X)-a(i,j-1,k,X))*ics[Y]
    };
#else
    return {0, -( a(i,j,k,Z)-a(i-1,j,k,Z))*ics[X], (a(i,j,k,Y)-a(i-1,j,k,Y))*ics[X]};
#endif

}

void E_curl(const Geometry geom,
            Box const& bx,
...
void B_curl(const Geometry geom,
            Box const& bx,
...
void push_B_E(const Geometry geom,
              Box const& bx,
              Array4<Real> const& B,
              Array4<const Real> const& E,
              double dt)
{
    push_ff<E_curl>(geom,bx,E,B,dt);
}

void push_E_B(const Geometry geom,
              Box const& bx,
              Array4<Real> const& E,
              Array4<const Real> const& B,
              double dt)
{
    push_ff<B_curl>(geom,bx,B,E,dt);
}

void push_V_E(CParticleType* particles,
              long np,
              const Geometry geom,
              Array4<Real const> const& E ,
              double dt)
{
    ...
    AMREX_D_TERM(
    lb[X] =geom.ProbLo(X); Ics[X]=_Ics[X];,
    lb[Y] =geom.ProbLo(Y); Ics[Y]=_Ics[Y];,
    lb[Z] =geom.ProbLo(Z); Ics[Z]=_Ics[Z];
    )
                ...
                [=] AMREX_GPU_DEVICE (long i)
    {
        ...
        Real pcoord[3] = {0,0,0};
        AMREX_D_TERM(
        pcoord[X]=(particles[i].pos(X) -lb[X])*Ics[X];,
        ...
        int coord[3] = {0,0,0};
        AMREX_D_TERM(
        coord[X]=floor(pcoord[0]);,
        coord[Y]=floor(pcoord[1]);,
...
void Symplectic::Theta_B(MFP* mfp, Real time, Real dt) const
{
    ...
    MultiFab fields_local(grids, dmap, +FieldDef::ConsIdx::NUM, interpolation_range+1, MFInfo(), mfp->Factory());
    mfp->FillPatch(*mfp, fields_local, interpolation_range+1, time, field->data_idx, 0, +FieldDef::ConsIdx::NUM);
    ...
    MultiFab::Copy(fields_global, fields_local, +FieldDef::ConsIdx::Dx, +FieldDef::ConsIdx::Dx, 3, 0);
...
void Symplectic::Theta_E(MFP* mfp, Real time, Real dt) const
{
    ...
    // only copy what we have updated
    MultiFab::Copy(fields_global, fields_local, +FieldDef::ConsIdx::Bx, +FieldDef::ConsIdx::Bx, 3, 0);
}

AMREX_GPU_HOST_DEVICE inline bool segment_reflect(int isPeriodic ,
                                                  int smallEnd,
                                                  int bigEnd,
                                                  Real * seg_points,
                                                  int * seg_idx)
...
AMREX_GPU_HOST_DEVICE inline void particle_reflect(CParticleType *p,
                                                   Real* seg_points,
                                                   int comp)
{
           ...
           bool generate_field)
{
    ...
    AMREX_D_TERM(
    lower_bound[X] =geom.ProbLo(X); Ics[X]=_Ics[X];,
    lower_bound[Y] =geom.ProbLo(Y); Ics[Y]=_Ics[Y];,
    lower_bound[Z] =geom.ProbLo(Z); Ics[Z]=_Ics[Z];
    )
...
//                [=] AMREX_GPU_DEVICE (long i)
    for (int i=0; i<np; ++i)
    {
        ...
        AMREX_D_TERM(
        coord[X]=floor((particles[i].pos(X) -lower_bound[X])*Ics[X]);,
