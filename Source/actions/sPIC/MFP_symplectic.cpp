...
bool Symplectic::registered =
  GetActionFactory().Register(Symplectic::tag, ActionBuilder<Symplectic>);
...
{
    BL_PROFILE("Symplectic::Symplectic");
    ...
    for (const auto& key_value_pair : state_names) {
        ...
        case State::StateType::Field: {
            if (has_field)
                Abort("Only one field state can be set for the symplectic action '" + name + "'");
...
{
    BL_PROFILE("Symplectic::apply_change");
    ...
    case 2:
        ...
        Theta_E(mfp, time, dt / 2);
        break;
    default: Abort("Time order of " + num2str(time_order) + " in action '" + name + "' is invalid");
    }
}
#endif
