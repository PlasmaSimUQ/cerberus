...
void BraginskiiCTU::calc_spatial_derivative(MFP* mfp, Vector<std::pair<int,MultiFab>>& dU, const Real time, const Real dt)
{
    ...
    for (int idx=0; idx<n_states; ++idx) {
        ...
        local_old[idx].define(grids, dmap, ns, num_grow, MFInfo(),mfp->Factory());

        // get a full array of data at this level
#ifdef AMREX_USE_EB
        EB2::IndexSpace::push(const_cast<EB2::IndexSpace*>(istate.eb2_index));
#endif
        mfp->FillPatch(*mfp, local_old[idx], num_grow, time, data_idx, 0, ns);
...
void BraginskiiCTU::calc_time_derivative(MFP* mfp, Vector<std::pair<int,MultiFab>>& dU, const Real time, const Real dt)
{
    ...
    MultiFab& cost = mfp->get_new_data(MFP::Cost_Idx);
    MultiFab& ion_data = mfp->get_data(ion_state->data_idx,time);
    MultiFab& field_data = mfp->get_data(field_state->data_idx,time);

    // need filled data for calculating electron temperature slopes
    const BoxArray& grids = mfp->boxArray();
    const DistributionMapping& dmap = mfp->DistributionMap();
    const int ng = 1; // number of ghost cells
    MultiFab electron_data(grids, dmap, nc_e, ng, MFInfo(),mfp->Factory());
    mfp->FillPatch(*mfp, electron_data, ng, time, electron_state->data_idx, 0, nc_e);
    ...
    for (MFIter mfi(cost); mfi.isValid(); ++mfi) {
        ...
        Array4<Real> const& ion_dU4 = dU[ion_state->data_idx].second.array(mfi);

        Array4<const Real> const& electron4 = electron_data.array(mfi);
        Array4<Real> const& electron_dU4 = dU[electron_state->data_idx].second.array(mfi);
