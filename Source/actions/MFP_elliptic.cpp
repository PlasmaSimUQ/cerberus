...
void Elliptic::solve_static_fields(MFP* mfp, const Real time)
{
    ...
    for (int ilev = 0; ilev < nlevels; ++ilev) {
        ...
        defined_phi[ilev].define(amr_grids[ilev], amr_dmap[ilev], 1, ngrow, MFInfo()
                         #ifdef AMREX_USE_EB
                                 ,*amr_factory[ilev]
                         #endif
                                 );
        ...
        defined_A[ilev].define(amr_grids[ilev], amr_dmap[ilev], 3, ngrow, MFInfo()
                       #ifdef AMREX_USE_EB
                               ,*amr_factory[ilev]
                       #endif
                               );
        ...
        defined_charge[ilev].define(amr_grids[ilev], amr_dmap[ilev], 1, ngrow, MFInfo()
                            #ifdef AMREX_USE_EB
                                    ,*amr_factory[ilev]
                            #endif
                                    );
        ...
        defined_current[ilev].define(amr_grids[ilev], amr_dmap[ilev], 3, ngrow, MFInfo()
                             #ifdef AMREX_USE_EB
                                     ,*amr_factory[ilev]
                             #endif
                                     );
        ...
        for (const auto& src_idx : field->associated_actions) {
            ...
            for (MFIter mfi(ilevel.get_new_data(MFP::Cost_Idx)); mfi.isValid(); ++mfi) {
                ...
                FArrayBox& local_cd = defined_charge[ilev][mfi]; local_cd.setVal(0.0);
                FArrayBox& local_J = defined_current[ilev][mfi]; local_J.setVal(0.0);
...
#ifdef AMREX_USE_EB
            EB_average_face_to_cellcenter(ifield, +FieldDef::ConsIdx::Dx, GetArrOfConstPtrs(bcoef[ilev]));
#else
            average_face_to_cellcenter(ifield, +FieldDef::ConsIdx::Dx, amrex::GetArrOfConstPtrs(bcoef[ilev]));
#endif
        }
    }
...
//            plot_FAB_2d(*(rhs_ptr[nlevels-1]),0,0, "J"+num2str(d), false, true);
            ...
            if (false) {
                for (int ilev = 0; ilev < nlevels; ++ilev) {
                    ...
                    MultiFab::Copy(ifield, A[ilev][d], 0, +FieldDef::ConsIdx::Bx+d, 1, 0); // magnetic vector potential
                ...
                for (int ilev = 0; ilev < nlevels; ++ilev) {
                    ...
                    MultiFab gradients(amr_grids[ilev], amr_dmap[ilev], 3, 0, MFInfo()
                   #ifdef AMREX_USE_EB
                                       ,*amr_factory[ilev]
                   #endif
                                       );
                    ...
                    switch (d) {
                    case 0:
                        MultiFab::Add(ifield, gradients, 1, +FieldDef::ConsIdx::Bz, 1, 0); //dAx/dy -> Bz
#if AMREX_SPACEDIM == 3
                        MultiFab::Subtract(ifield, gradients, 2, +FieldDef::ConsIdx::By, 1, 0); //dAx/dz -> By
#endif
                        break;
                    case 1:
                        MultiFab::Subtract(ifield, gradients, 0, +FieldDef::ConsIdx::Bz, 1, 0); //dAy/dx -> Bz
#if AMREX_SPACEDIM == 3
                        MultiFab::Add(ifield, gradients, 2, +FieldDef::ConsIdx::By, 1, 0); //dAy/dz -> Bx
#endif
                        break;
                    case 2:
                        MultiFab::Add(ifield, gradients, 0, +FieldDef::ConsIdx::By, 1, 0); //dAz/dx -> By
                        MultiFab::Subtract(ifield, gradients, 1, +FieldDef::ConsIdx::Bx, 1, 0); //dAz/dy -> Bx
...
void Elliptic::project_divergence(MFP* mfp, const Real time)
{
    ...
    for (int ilev = 0; ilev < nlevels; ++ilev) {
...
#ifdef AMREX_USE_EB
        EBFArrayBoxFactory const* amr_factory = mfp->get_eb_data(ilev,field->global_idx).ebfactory.get();
#endif

        charge_density[ilev].define(amr_grids[ilev], amr_dmap[ilev], 1, ngrow, MFInfo()
                            #ifdef AMREX_USE_EB
                                    ,*amr_factory
                            #endif
                                    );
        ...
        for (const auto& src_idx : field->associated_actions) {
            ...
            for (MFIter mfi(ilevel.get_new_data(MFP::Cost_Idx)); mfi.isValid(); ++mfi) {
                ...
                FArrayBox& local_cd = charge_density[ilev][mfi]; local_cd.setVal(0.0);
    ...
    // now that we have the charge density over all levels do the actual solve
    solve_divergence(mfp, field->data_idx, +FieldDef::ConsIdx::Dx, +FieldDef::ConsIdx::phi, field->boundary_conditions.phys_fill_bc[+FieldDef::ConsIdx::Dx], GetVecOfPtrs(charge_density));
                                ...
                                Vector<MultiFab*> S_cc_ptr)
{
    ...
    for (int ilev = 0; ilev < nlevels; ++ilev) {
        ...
        //
        field_data[ilev].define(amr_grids[ilev], amr_dmap[ilev], AMREX_SPACEDIM, 1, MFInfo(), ilevel.Factory());
