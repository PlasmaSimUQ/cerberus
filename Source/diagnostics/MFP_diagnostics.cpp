        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    s4(i, j, k, n) +=
                      d4(i, j, k, n) - d4(i + index[0], j + index[1], k + index[2], n);
                }
            }
        }
    }
...
void plot_FAB_1d(
  const Box& box, const FArrayBox& src, const Vector<int> N, std::string title, bool block)
{
    const Dim3 lo = amrex::lbound(box);
    const Dim3 hi = amrex::ubound(box);
    ...
    for (int n = 0; n < N.size(); ++n) {
        data.clear();
        for (int i = lo.x; i <= hi.x; ++i) { data.push_back(src4(i, 0, 0, N[n])); }
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                std::map<std::string, std::string> string_args = {{"color", "k"},
                                                                  {"linestyle", "dotted"}};
...
void plot_FAB_eb(
  const FArrayBox& src, const FArrayBox& pts, const int n, std::string title, bool block)
{
...
void plot_FABs_2d(const std::map<std::string, FArrayBox>& src, const int fi, bool log, bool block)
{
    ...
    for (const auto& FAB : src) {
        ...
        xticks.resize(nc + 1);
        std::iota(xticks.begin(), xticks.end(), lo.x);
        for (const auto& i : xticks) { xlabels.push_back(std::to_string(i)); }

        yticks.resize(nr + 1);
        std::iota(yticks.begin(), yticks.end(), lo.y);
        for (const auto& i : yticks) { ylabels.push_back(std::to_string(i)); }
        ...
        plt::imshow(data.data(),
                    ...
                    1,
                    {{"origin", "lower"}},
                    {lo.x - 0.5f, hi.x + 0.5f, lo.y - 0.5f, hi.y + 0.5f});
...
void plot_FABs_2d(
  const Vector<FArrayBox>& src, const int fi, std::string title, bool log, bool block)
{
    if (block) plt::figure_size(1200, 780);
...
void plot_FAB_2d(
  const MultiFab& src, const int fi, const int shrink, std::string title, bool log, bool block)
{
    plt::figure_size(1200, 780);
    ...
    plt::imshow(data.data(),
                ...
                1,
                {{"origin", "lower"}},
                {xlo - 0.5f, xhi + 0.5f, ylo - 0.5f, yhi + 0.5f});
