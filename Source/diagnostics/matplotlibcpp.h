...
template<typename Numeric>
bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::map<std::string, std::string>& keywords, const std::map<std::string, double>& keyvalues)
{
    ...
    for(std::map<std::string, double>::const_iterator it = keyvalues.begin(); it != keyvalues.end(); ++it)
    {
        PyDict_SetItem(kwargs, PyString_FromString(it->first.c_str()), PyFloat_FromDouble((it->second)));
    }
          ...
          double alpha=1.0, bool cumulative=false)
{
...
#ifndef WITHOUT_NUMPY
    namespace internal {
    inline void imshow(void *ptr, const NPY_TYPES type, const int rows, const int columns, const int colors, const std::map<std::string, std::string> &keywords, std::vector<float> extents={})
        {
            ...
            if (!extents.empty()) {
                PyObject *xt = PyTuple_New(4);
                PyTuple_SetItem(xt, 0, PyFloat_FromDouble(extents[0]));
                PyTuple_SetItem(xt, 1, PyFloat_FromDouble(extents[1]));
                PyTuple_SetItem(xt, 2, PyFloat_FromDouble(extents[2]));
                PyTuple_SetItem(xt, 3, PyFloat_FromDouble(extents[3]));
                PyDict_SetItem(kwargs, PyUnicode_FromString("extent"), xt);
            }
    ...
    inline void imshow(const unsigned char *ptr, const int rows, const int columns, const int colors, const std::map<std::string, std::string> &keywords = {}, const std::vector<float> extents={})
    {
        internal::imshow((void *) ptr, NPY_UINT8, rows, columns, colors, keywords, extents);
    }

    inline void imshow(const float *ptr, const int rows, const int columns, const int colors, const std::map<std::string, std::string> &keywords = {}, const std::vector<float> extents={})
    {
        internal::imshow((void *) ptr, NPY_FLOAT, rows, columns, colors, keywords, extents);
    }
    ...
    inline void pcolormesh(std::vector<float>& x, std::vector<float>& y, std::vector<float>& z, const std::map<std::string, std::string> &keywords={})
        {
            const NPY_TYPES type = NPY_FLOAT;

            detail::_interpreter::get();    //interpreter needs to be initialized for the numpy commands to work

            // construct args
            void *x_ptr = static_cast<void*>(x.data());
            void *y_ptr = static_cast<void*>(y.data());
            void *z_ptr = static_cast<void*>(z.data());

            npy_intp dims1[1] = { (npy_intp)x.size()};
            npy_intp dims2[1] = { (npy_intp)y.size()};
            npy_intp dims3[2] = { (npy_intp)y.size(), (npy_intp)x.size()};
            PyObject *args = PyTuple_New(3);
            PyTuple_SetItem(args, 0, PyArray_SimpleNewFromData(1, dims1, type, x_ptr));
            PyTuple_SetItem(args, 1, PyArray_SimpleNewFromData(1, dims2, type, y_ptr));
            PyTuple_SetItem(args, 2, PyArray_SimpleNewFromData(2, dims3, type, z_ptr));
            ...
            if (!res) {
                ...
                throw std::runtime_error(pvalue->ob_type->tp_doc);
            }
            Py_DECREF(res);
        }
...
{
    PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_colorbar, detail::_interpreter::get().s_python_empty_tuple);
...
template<typename Numeric>
void text(Numeric x, Numeric y, const std::string& s = "", std::map<std::string, std::string> arg={})
{
    PyObject* args = PyTuple_New(3);
    ...
    for (const auto a : arg) {
        PyDict_SetItemString(kwargs, a.first.c_str(), PyString_FromString(a.second.c_str()));
    }
...
inline bool plot(const std::vector<double>& x, const std::vector<double>& y, const std::map<std::string, std::string>& keywords, const std::map<std::string, double>& keyvalues) {
    return plot<double>(x,y,keywords,keyvalues);
}
