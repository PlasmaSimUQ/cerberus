...
{
    BL_PROFILE("EulerianState::set_reconstruction");
    ...
    if (is_transported() && rec == "null")
        Abort("Reconstruction option required for state '" + name + "'. Options are " +
...
{
    BL_PROFILE("EulerianState::set_reflux");
...
{
    BL_PROFILE("EulerianState::init_data");
...
{
    BL_PROFILE("EulerianState::get_plot_output_names");
...
{
    BL_PROFILE("EulerianState::get_plot_output");
...
{
    BL_PROFILE("EulerianState::get_allowed_time_step");
        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (int n = 0; n < n_cons(); ++n) { U[n] = data4(i, j, k, n); }
    ...
    for (int i = 1; i < AMREX_SPACEDIM; ++i) { dt = std::min(dt, dx[i] / max_speed); }
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (vfrac4(i, j, k) == 0.0) {
                    ...
                    for (const auto& index : grab) {
                        ...
                        if ((lo.x > ii) || (ii > hi.x) || (lo.y > jj) || (jj > hi.y) ||
                            (lo.z > kk) || (kk > hi.z))
                            continue;

                        const Real vf = vfrac4(ii, jj, kk);
                        if (vf > 0.0) {
                            for (int n = 0; n < n_cons(); ++n) { U[n] += vf * s4(ii, jj, kk, n); }
                ...
                // copy into primitive
                for (int n = 0; n < n_prim(); ++n) { p4(i, j, k, n) = Q[n]; }
            }
        }
    }
...
{
    BL_PROFILE("EulerianState::update_boundary_cells");
            ...
            for (int k = L.kmin; k <= L.kmax; ++k) {
                ...
                for (int j = L.jmin; j <= L.jmax; ++j) {
                    ...
                    for (int i = L.imin; i <= L.imax; ++i) {
                        ...
                        if (L.dir != 0) { grab[0] = i; }
#ifdef AMREX_USE_EB
                        if (vfrac4(grab[0], grab[1], grab[2]) == 0.0) { continue; }
#endif
                        ...
                        // update the primitives from our UDFs, but only those that are valid
                        // functions
                        for (int n = 0; n < prim_names.size(); ++n) {
                            const Optional3D1VFunction& f =
                              boundary_conditions.get(L.lo_hi, L.dir, prim_names[n]);
                            if (f.is_valid()) { p4(i, j, k, n) = f(Q); }
                        }
                    }
                }
            }
                    ...
                    for (int i = lo.x; i <= hi.x; ++i) {
                        ...
                        if (check_eb) {
                            ...
                            if (f4(i, j, k).isCovered() ||
                                check_covered_stencil(f4,
                                                      i,
                                                      j,
                                                      k,
                                                      d,
                                                      reconstructor->stencil_length)) {
                                continue;
                            }
                        }
#endif
...
void EulerianState::calc_time_averaged_faces(const Box& box,
                                             ...
                                             Real dt) const
{
    BL_PROFILE("EulerianState::calc_time_averaged_faces");
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                for (int n = 0; n < n_prim(); ++n) { Q[n] = p4(i, j, k, n); }
                ...
                for (int d = 0; d < AMREX_SPACEDIM; ++d) {
                    ...
                    for (int n = 0; n < n_prim(); ++n) {
                        ...
                        lo4(i, j, k, n) +=
                          local_c[d] * dt_2dx *
                          (hi_face - lo_face + (1 - local_c[d] * ft * dt_2dx) * a6);
                        hi4(i, j, k, n) -=
                          local_c[d] * dt_2dx *
                          (hi_face - lo_face - (1 - local_c[d] * ft * dt_2dx) * a6);
                    }
...
{
    BL_PROFILE("EulerianState::face_bc");
    ...
    if (do_all) {
        ...
        for (int n = 0; n < n_prim(); ++n) {
            ...
            for (const auto& L : limits) {
                if (((L.lo_hi == 0) && (bc.lo(dir) == BCType::foextrap)) ||
            ...
            for (int k = L.kmin; k <= L.kmax; ++k) {
                ...
                for (int j = L.jmin; j <= L.jmax; ++j) {
                    ...
                    for (int i = L.imin; i <= L.imax; ++i) {
                        ...
                        for (int n = 0; n < prim_names.size(); ++n) {
                            const Optional3D1VFunction& f =
                              boundary_conditions.get(L.lo_hi, dir, prim_names[n]);
                            if (f.is_valid()) { d4(i, j, k, n) = f(Q); }
                        }
                    }
                }
            }
...
void EulerianState::update_face_prim(const Box& box,
                                     const Geometry& geom,
        ...
        for (int k = flo.z; k <= fhi.z; ++k) {
            for (int j = flo.y; j <= fhi.y; ++j) {
                ...
                for (int i = flo.x; i <= fhi.x; ++i) {
                    ...
                    if (f4(i, j, k).isCovered() &&
                        f4(i - index[0], j - index[1], k - index[2]).isCovered())
                        continue;
#endif
                    ...
                    if (shock_detector) { shk = shock_detector->solve(L, R); }
...
#ifdef AMREX_USE_EB
                        if (check_eb) {
                            // only calculate corrections for faces where the stencil of fluxes is
                            // full

                            // a) this interface has a valid flux
                            if (f4(i, j, k).isDisconnected(-idx1[0], -idx1[1], -idx1[2])) continue;

                            // b) right cell has valid fluxes hi and lo
                            if (f4(i, j, k).isDisconnected(idx2[0], idx2[1], idx2[2])) continue;
                            if (f4(i, j, k).isDisconnected(-idx2[0], -idx2[1], -idx2[2])) continue;

                            // c) left cell has valid fluxes hi and lo
                            if (f4(i - idx1[0], j - idx1[1], k - idx1[2])
                                  .isDisconnected(idx2[0], idx2[1], idx2[2]))
                                continue;
                            if (f4(i - idx1[0], j - idx1[1], k - idx1[2])
                                  .isDisconnected(-idx2[0], -idx2[1], -idx2[2]))
                                continue;
...
#ifdef AMREX_USE_EB
Real EulerianState::interp2d(
  Real cym, Real cy0, Real cyp, Real czm, Real cz0, Real czp, Eigen::Matrix3d v)
{
    BL_PROFILE("EulerianState::interp2d");
...
void EulerianState::calc_wall_fluxes(const Box& box,
                                     ...
                                     const Real dt) const
{
    BL_PROFILE("EulerianState::calc_wall_fluxes");
    ...
    for (size_t i = 0; i < all_prim.size(); ++i) {
        if (all_prim[i]) { p4[i] = all_prim[i]->array(); }
    }
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    for (int d = 0; d < AMREX_SPACEDIM; ++d) {
                        ...
                        if (cflag.isSingleValued()) { wflux = wflux4[d](i, j, k, n); }
...
{
    return i >= lo.x && i <= hi.x && j >= lo.y && j <= hi.y && k >= lo.z && k <= hi.z;
}

// https://stackoverflow.com/a/53283994
struct ArrayHasher {
    int N;
    ArrayHasher(int n) { N = n; }
    int operator()(const Array<int, 3>& V) const { return V[0] + N * (V[1] + N * V[2]); }
};
                                ...
                                const IArrayBox& levmsk) const
{
    ...
    for (int d = 0; d < AMREX_SPACEDIM; ++d) { afrac4[d] = afrac[d]->array(); }
        ...
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (vf < merge_threshold) {
                    ...
                    if (cells.find(cell_idx) != cells.end()) continue;
                    ...
                    if (new_super_cell) {
                        for (const auto& cell : super_cell) { cells[cell] = merge.size(); }

                        merge.push_back(super_cell);
                    }

                } else if (cflag.isSingleValued() && as_fine) {
                    for (int n = 0; n < nc; ++n) { dm_as_fine4(i, j, k, n) = 0.0; }
                }
            }
        }
    }
        ...
        for (int mi = 0; mi < nsuper; ++mi) {
            ...
            for (const auto& cell_idx : mc) {
                ...
                if (as_fine && (levmsk4(i, j, k) == LevelMask::NotCovered)) {
                    dm_as_fine4(i, j, k, n) =
                      du_merge - du4(cell_idx[0], cell_idx[1], cell_idx[2], n);
                }
            }
        }
    }
...
bool EulerianState::check_covered_stencil(
  Array4<const EBCellFlag> const& flag, int i, int j, int k, int d, int stencil_length)
{
    BL_PROFILE("EulerianState::check_covered_stencil");
...
void EulerianState::calc_divergence(const Box& box,
                                    ...
                                    const Real dt) const
{
    BL_PROFILE("EulerianState::calc_divergence");
    ...
    for (int d = 0; d < AMREX_SPACEDIM; ++d) { flux4[d] = fluxes[d].array(); }
        ...
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (check_eb) {
                    // covered cell doesn't need calculating
                    if (f4(i, j, k).isCovered() ||
                        check_covered_stencil(f4, i, j, k, dim, reco.stencil_length)) {
                        s4(i, j, k) = 0.0;
                        continue;
                    }
                }
                ...
                // perform reconstruction
                s4(i, j, k, slope_idx) =
                  reco.get_slope(stencil) / dx[dim];  // account for local cell size
            }
        }
    }
...
{
    BL_PROFILE("EulerianState::write_info");
