...
Vector<set_bc> HydroState::bc_set = {&set_scalar_bc,
                                     &set_x_vel_bc,
                                     &set_y_vel_bc,
                                     &set_z_vel_bc,
                                     &set_scalar_bc,
...
{
    BL_PROFILE("HydroState::HydroState");
...
{
    BL_PROFILE("HydroState::set_viscosity");
...
{
    BL_PROFILE("HydroState::init_from_number_density");

    Vector<Real> Q(n_prim());

    Real nd = other_functions["nd"](data);
    if (gas->mass_const) return nd * gas->mass[0];
...
void HydroState::set_udf()
{
    ...
    if (alpha.valid()) {
        ...
        if (alpha_list.size() != n_tracers)
            Abort("Incorrect number of 'alpha' defined for state '" + name +
                  "' given the number of components, need " + num2str(n_tracers));
        ...
        for (int i = i_start; i < i_stop; ++i) {
            ...
            if (get_udf(alpha_list[i - i_start + 1], v, 0.0)) { functions[i] = v; }
        }
    } else {
        const int i_stop = i_start + n_tracers;
        functions.resize(i_stop);
        for (int i = i_start; i < i_stop; ++i) { functions[i].set_value(0.0); }
    }
...
{
    BL_PROFILE("HydroState::set_flux");
...
{
    BL_PROFILE("HydroState::set_shock_detector");
...
{
    BL_PROFILE("HydroState::set_refinement");
...
void HydroState::init_from_lua()
{
    ...
    for (int i = 0; i < n_tracers; ++i) { bc_set.push_back(&set_scalar_bc); }
    ...
    for (int ax = 0; ax < AMREX_SPACEDIM; ++ax) {
        for (int lh = 0; lh < 2; ++lh) {
            std::string side_bc =
              state_def["bc"][dir_name[ax]][side_name[lh]]["fill_hydro_bc"].get_or<std::string>(
                "outflow");
            ...
            for (int j = 0; j < N; ++j) {
                ...
                const sol::object v =
                  state_def["bc"][dir_name[ax]][side_name[lh]][hydro_var[j]].get_or(sol::object());
                ...
                if (i_side_bc == PhysBCType::inflow && !f.is_valid()) {
                    Abort("Setting 'fill_hydro_bc = inflow' requires all primitive variables to be "
                          "defined, '" +
                          hydro_var[j] + "' is not defined");
                }
            }

            // now handle the tracers

            const sol::object alpha_funcs =
              state_def["bc"][dir_name[ax]][side_name[lh]][multicomp_prim_name].get_or(
                sol::object());

            if (alpha_funcs.valid()) {
                if (alpha_funcs.get_type() == sol::type::table) {
                    if (alpha_funcs.as<sol::table>().size() < n_tracers) {
                        Abort("Not enough boundary conditions specified for alpha in state '" +
                              name + "'");
                    }
                }
            }
...
#ifdef AMREX_USE_EB
            bool is_symmetry = (i_side_bc == PhysBCType::symmetry) ||
                               (i_side_bc == PhysBCType::slipwall) ||
                               (i_side_bc == PhysBCType::noslipwall);
            if (lh == 0) {
...
{
    BL_PROFILE("HydroState::variable_setup");
    ...
    for (int icomp = +HydroDef::ConsIdx::NUM; icomp < n_cons(); ++icomp) {
        comp_names[icomp] =
          multicomp_cons_name + "_" + num2str(icomp - +HydroDef::ConsIdx::NUM) + "-" + name;
    }
...
{
    BL_PROFILE("HydroState::init_data");
        ...
        for (int k = lo.z; k <= hi.z; ++k) {
            ...
            for (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    if (cflag.isCovered()) {
                        for (int n = 0; n < n_cons(); ++n) { S4(i, j, k, n) = 0.0; }
                        continue;
                    }
#endif
...
{
    BL_PROFILE("HydroState::get_plot_output_names");
...
{
    BL_PROFILE("HydroState::get_plot_output");
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (!cons_tags.empty()) {
                    for (const auto& var : cons_tags) { out4[var.first](i, j, k) = S[var.second]; }
                }

                if (!prim_tags.empty()) {
                    gas->cons2prim(S, Q);

                    for (const auto& var : prim_tags) { out4[var.first](i, j, k) = Q[var.second]; }
                }
            }
        }
    }
...
{
    BL_PROFILE("HydroState::get_allowed_time_step");

    Real dt = EulerianState::get_allowed_time_step(mfp);

    // check for any viscous time step limitation
    if (viscous) { dt = std::min(dt, viscous->get_min_dt(mfp)); }
...
// #endif
//
//                 for (int n = 0; n < n_prim(); ++n) { Q[n] = src4(i, j, k, n); }
//
...
//                             if (f4(i + stencil_index[0], j + stencil_index[1], k +
//                             stencil_index[2])
//                                   .isCovered()) {
//                                 skip = true;
//                                 break;
...
//                         for (int s = 0; s < reconstructor->stencil_length; ++s) {
//                             stencil_index[d] = s - offset;
//                             stencil[s] = src4(i + stencil_index[0],
//                                               j + stencil_index[1],
//                                               k + stencil_index[2],
//                                               n);
//                         }
...
//                     if (std::abs(phi_rho) > 2 * rho) { phi_rho = 2 * sign(phi_rho, 0.0) * rho; }
//
...
//
//                     // epsilon -> pressure
//                     lo4(i, j, k, +HydroDef::PrimIdx::Prs) = (eps - 0.5 * phi_eps) * (gam_lo
//                     - 1.0); hi4(i, j, k, +HydroDef::PrimIdx::Prs) = (eps + 0.5 * phi_eps) *
//                     (gam_hi - 1.0);
//
//                     // Temperature (calculate from pressure and density)
//                     lo4(i, j, k, +HydroDef::PrimIdx::Temp) =
//                       lo4(i, j, k, +HydroDef::PrimIdx::Prs) /
//                       (rho_lo / gas->get_mass_from_prim(alphas_lo, 0));
//                     hi4(i, j, k, +HydroDef::PrimIdx::Temp) =
//                       hi4(i, j, k, +HydroDef::PrimIdx::Prs) /
//                       (rho_hi / gas->get_mass_from_prim(alphas_hi, 0));
//
//                     // gamma
...
//
//                     // specific heat
//                     lo4(i, j, k, +HydroDef::PrimIdx::SpHeat) = gas->get_cp_from_prim(alphas_lo,
//                     0); hi4(i, j, k, +HydroDef::PrimIdx::SpHeat) =
//                     gas->get_cp_from_prim(alphas_hi, 0);
//                 }
//             }
//         }
//     }
...
{
    BL_PROFILE("HydroState::calc_diffusion_terms");
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                for (int n = 0; n < n_prim(); ++n) { Q[n] = prim4(i, j, k, n); }
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x + 1; ++i) {
                ...
                dTdx = (d4(i, j, k, +HydroViscous::CoeffIdx::Temp) -
                        d4(i - 1, j, k, +HydroViscous::CoeffIdx::Temp)) *
                       dxinv[0];

                dudx = (p4(i, j, k, +HydroDef::PrimIdx::Xvel) -
                        p4(i - 1, j, k, +HydroDef::PrimIdx::Xvel)) *
                       dxinv[0];
                dvdx = (p4(i, j, k, +HydroDef::PrimIdx::Yvel) -
                        p4(i - 1, j, k, +HydroDef::PrimIdx::Yvel)) *
                       dxinv[0];
                dwdx = (p4(i, j, k, +HydroDef::PrimIdx::Zvel) -
                        p4(i - 1, j, k, +HydroDef::PrimIdx::Zvel)) *
                       dxinv[0];

#if AMREX_SPACEDIM >= 2
                dudy = (p4(i, j + 1, k, +HydroDef::PrimIdx::Xvel) +
                        p4(i - 1, j + 1, k, +HydroDef::PrimIdx::Xvel) -
                        p4(i, j - 1, k, +HydroDef::PrimIdx::Xvel) -
                        p4(i - 1, j - 1, k, +HydroDef::PrimIdx::Xvel)) *
                       (0.25 * dxinv[1]);
                ...
                muf = 0.5 * (d4(i, j, k, +HydroViscous::CoeffIdx::Mu) +
                             d4(i - 1, j, k, +HydroViscous::CoeffIdx::Mu));
                ...
                fluxX(i, j, k, +HydroDef::ConsIdx::Eden) -=
                  0.5 * ((p4(i, j, k, +HydroDef::PrimIdx::Xvel) +
                          p4(i - 1, j, k, +HydroDef::PrimIdx::Xvel)) *
                           tauxx +
                         (p4(i, j, k, +HydroDef::PrimIdx::Yvel) +
                          p4(i - 1, j, k, +HydroDef::PrimIdx::Yvel)) *
                           tauxy +
                         (p4(i, j, k, +HydroDef::PrimIdx::Zvel) +
                          p4(i - 1, j, k, +HydroDef::PrimIdx::Zvel)) *
                           tauxz +
                         (d4(i, j, k, +HydroViscous::CoeffIdx::Kappa) +
                          d4(i - 1, j, k, +HydroViscous::CoeffIdx::Kappa)) *
                           dTdx);
            }
        }
    }
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y + 1; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                dTdy = (d4(i, j, k, +HydroViscous::CoeffIdx::Temp) -
                        d4(i, j - 1, k, +HydroViscous::CoeffIdx::Temp)) *
                       dxinv[1];
                dudy = (p4(i, j, k, +HydroDef::PrimIdx::Xvel) -
                        p4(i, j - 1, k, +HydroDef::PrimIdx::Xvel)) *
                       dxinv[1];
                dvdy = (p4(i, j, k, +HydroDef::PrimIdx::Yvel) -
                        p4(i, j - 1, k, +HydroDef::PrimIdx::Yvel)) *
                       dxinv[1];
                dwdy = (p4(i, j, k, +HydroDef::PrimIdx::Zvel) -
                ...
                dudx = (p4(i + 1, j, k, +HydroDef::PrimIdx::Xvel) +
                        ...
                        p4(i - 1, j - 1, k, +HydroDef::PrimIdx::Xvel)) *
                       (0.25 * dxinv[0]);
                dvdx = (p4(i + 1, j, k, +HydroDef::PrimIdx::Yvel) +
                        p4(i + 1, j - 1, k, +HydroDef::PrimIdx::Yvel) -
                        p4(i - 1, j, k, +HydroDef::PrimIdx::Yvel) -
                        p4(i - 1, j - 1, k, +HydroDef::PrimIdx::Yvel)) *
                       (0.25 * dxinv[0]);
                ...
                divu = dudx + dvdy + dwdz;
                muf = 0.5 * (d4(i, j, k, +HydroViscous::CoeffIdx::Mu) +
                             d4(i, j - 1, k, +HydroViscous::CoeffIdx::Mu));
                ...
                fluxY(i, j, k, +HydroDef::ConsIdx::Eden) -=
                  0.5 * ((p4(i, j, k, +HydroDef::PrimIdx::Xvel) +
                          p4(i, j - 1, k, +HydroDef::PrimIdx::Xvel)) *
                           tauxy +
                         (p4(i, j, k, +HydroDef::PrimIdx::Yvel) +
                         ...
                         (p4(i, j, k, +HydroDef::PrimIdx::Zvel) +
                          p4(i, j - 1, k, +HydroDef::PrimIdx::Zvel)) *
                           tauyz +
                         (d4(i, j, k, +HydroViscous::CoeffIdx::Kappa) +
                          d4(i, j - 1, k, +HydroViscous::CoeffIdx::Kappa)) *
                           dTdy);
            }
        }
    }
    ...
    for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y + 1; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                fluxZ(i, j, k, +HydroDef::ConsIdx::Eden) -=
                  0.5 * ((p4(i, j, k, +HydroDef::PrimIdx::Xvel) +
                          p4(i, j, k - 1, +HydroDef::PrimIdx::Xvel)) *
                           tauxz +
                         (p4(i, j, k, +HydroDef::PrimIdx::Yvel) +
                         ...
                         (p4(i, j, k, +HydroDef::PrimIdx::Zvel) +
                          p4(i, j, k - 1, +HydroDef::PrimIdx::Zvel)) *
                           tauzz +
                         (d4(i, j, k, +HydroViscous::CoeffIdx::Kappa) +
                          d4(i, j, k - 1, +HydroViscous::CoeffIdx::Kappa)) *
                           dTdz);
            }
        }
    }
...
#ifdef AMREX_USE_EB
void HydroState::calc_viscous_fluxes_eb(const Box& box,
                                        Array<FArrayBox, AMREX_SPACEDIM>& fluxes,
                                        const FArrayBox& prim,
    #ifdef AMREX_USE_EB
                                        const EBCellFlagFab& flag,
                                        ...
                                        const Real* dx) const
{
    BL_PROFILE("HydroState::calc_viscous_fluxes_eb");
    ...
    for (int k = lo.z - AMREX_D_PICK(0, 0, 1); k <= hi.z + AMREX_D_PICK(0, 0, 1); ++k) {
        for (int j = lo.y - AMREX_D_PICK(0, 1, 1); j <= hi.y + AMREX_D_PICK(0, 1, 1); ++j) {
            ...
            for (int i = lo.x; i <= hi.x + 1; ++i) {
                ...
                if (covered || other_covered || !connected) continue;
                ...
                dudy = (0.5 * dxinv[1]) * ((p4(i, jhip, k, +HydroDef::PrimIdx::Xvel) -
                                            p4(i, jhim, k, +HydroDef::PrimIdx::Xvel)) *
                                             whi +
                                           (p4(i - 1, jlop, k, +HydroDef::PrimIdx::Xvel) -
                                            p4(i - 1, jlom, k, +HydroDef::PrimIdx::Xvel)) *
                                             wlo);
                dvdy = (0.50 * dxinv[1]) * ((p4(i, jhip, k, +HydroDef::PrimIdx::Yvel) -
                                             p4(i, jhim, k, +HydroDef::PrimIdx::Yvel)) *
                                              whi +
                                            (p4(i - 1, jlop, k, +HydroDef::PrimIdx::Yvel) -
                                             p4(i - 1, jlom, k, +HydroDef::PrimIdx::Yvel)) *
                                              wlo);
                ...
                dudz = (0.5 * dxinv[2]) * ((p4(i, j, khip, +HydroDef::PrimIdx::Xvel) -
                                            p4(i, j, khim, +HydroDef::PrimIdx::Xvel)) *
                                             whi +
                                           (p4(i - 1, j, klop, +HydroDef::PrimIdx::Xvel) -
                                            p4(i - 1, j, klom, +HydroDef::PrimIdx::Xvel)) *
                                             wlo);
                dwdz = (0.5 * dxinv[2]) * ((p4(i, j, khip, +HydroDef::PrimIdx::Zvel) -
                                            p4(i, j, khim, +HydroDef::PrimIdx::Zvel)) *
                                             whi +
                                           (p4(i - 1, j, klop, +HydroDef::PrimIdx::Zvel) -
                                            p4(i - 1, j, klom, +HydroDef::PrimIdx::Zvel)) *
                                             wlo);
    ...
    for (int k = lo.z - AMREX_D_PICK(0, 0, 1); k <= hi.z + AMREX_D_PICK(0, 0, 1); ++k) {
        for (int j = lo.y; j <= hi.y + 1; ++j) {
            ...
            for (int i = lo.x - 1; i <= hi.x + 1; ++i) {
                ...
                dudx = (0.5 * dxinv[0]) * ((p4(ihip, j, k, +HydroDef::PrimIdx::Xvel) -
                                            p4(ihim, j, k, +HydroDef::PrimIdx::Xvel)) *
                                             whi +
                                           (p4(ilop, j - 1, k, +HydroDef::PrimIdx::Xvel) -
                                            p4(ilom, j - 1, k, +HydroDef::PrimIdx::Xvel)) *
                                             wlo);
                dvdx = (0.5 * dxinv[0]) * ((p4(ihip, j, k, +HydroDef::PrimIdx::Yvel) -
                                            p4(ihim, j, k, +HydroDef::PrimIdx::Yvel)) *
                                             whi +
                                           (p4(ilop, j - 1, k, +HydroDef::PrimIdx::Yvel) -
                                            p4(ilom, j - 1, k, +HydroDef::PrimIdx::Yvel)) *
                                             wlo);
                ...
                dvdz = (0.5 * dxinv[2]) * ((p4(i, j, khip, +HydroDef::PrimIdx::Yvel) -
                                            p4(i, j, khim, +HydroDef::PrimIdx::Yvel)) *
                                             whi +
                                           (p4(i, j - 1, klop, +HydroDef::PrimIdx::Yvel) -
                                            p4(i, j - 1, klom, +HydroDef::PrimIdx::Yvel)) *
                                             wlo);
                dwdz = (0.5 * dxinv[2]) * ((p4(i, j, khip, +HydroDef::PrimIdx::Zvel) -
                                            p4(i, j, khim, +HydroDef::PrimIdx::Zvel)) *
                                             whi +
                                           (p4(i, j - 1, klop, +HydroDef::PrimIdx::Zvel) -
                                            p4(i, j - 1, klom, +HydroDef::PrimIdx::Zvel)) *
                                             wlo);
        ...
        for (int j = lo.y - 1; j <= hi.y + 1; ++j) {
            ...
            for (int i = lo.x - 1; i <= hi.x + 1; ++i) {
                ...
                dudx = (0.5 * dxinv[0]) * ((p4(ihip, j, k, +HydroDef::PrimIdx::Xvel) -
                                            p4(ihim, j, k, +HydroDef::PrimIdx::Xvel)) *
                                             whi +
                                           (p4(ilop, j, k - 1, +HydroDef::PrimIdx::Xvel) -
                                            p4(ilom, j, k - 1, +HydroDef::PrimIdx::Xvel)) *
                                             wlo);
                dwdx = (0.5 * dxinv[0]) * ((p4(ihip, j, k, +HydroDef::PrimIdx::Zvel) -
                                            p4(ihim, j, k, +HydroDef::PrimIdx::Zvel)) *
                                             whi +
                                           (p4(ilop, j, k - 1, +HydroDef::PrimIdx::Zvel) -
                                            p4(ilom, j, k - 1, +HydroDef::PrimIdx::Zvel)) *
                                             wlo);
                ...
                dvdy = (0.5 * dxinv[1]) * ((p4(i, jhip, k, +HydroDef::PrimIdx::Yvel) -
                                            p4(i, jhim, k, +HydroDef::PrimIdx::Yvel)) *
                                             whi +
                                           (p4(i, jlop, k - 1, +HydroDef::PrimIdx::Yvel) -
                                            p4(i, jlom, k - 1, +HydroDef::PrimIdx::Yvel)) *
                                             wlo);
                dwdy = (0.5 * dxinv[1]) * ((p4(i, jhip, k, +HydroDef::PrimIdx::Zvel) -
                                            p4(i, jhim, k, +HydroDef::PrimIdx::Zvel)) *
                                             whi +
                                           (p4(i, jlop, k - 1, +HydroDef::PrimIdx::Zvel) -
                                            p4(i, jlom, k - 1, +HydroDef::PrimIdx::Zvel)) *
                                             wlo);
        ...
        for (int j = lo.y; j <= hi.y; ++j) {
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (vfrac4(i, j, k) == 0.0) {
                    ...
                    for (int n = 0; n < n_cons(); ++n) { U[n] = s4(i, j, k, n); }
#ifdef AMREX_USE_EB
                }
#endif
                ...
                if (get_charge) { cd4(i, j, k) += U[+HydroDef::ConsIdx::Density] * q / m; }
...
{
    BL_PROFILE("HydroState::write_info");
    ...
    if (viscous) {
        ...
        for (const auto& cf : coeffs) { grp[cf.first] = cf.second; }
    }
}
