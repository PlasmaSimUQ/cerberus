...
void MHDState::set_eb_bc(const sol::table& bc_def)
{
    ...
    if (bc_type == MHDSlipWall::tag) {
        ...
        eb_bcs.push_back(
          std::unique_ptr<MHDBoundaryEB>(new DirichletWallMHD(global_idx, flux, bc_def)));
    } else {
...
DirichletWallMHD::DirichletWallMHD(int idx, MHDRiemannSolver* flux, const sol::table& bc_def)
{
    BL_PROFILE("DirichletWallMHD::DirichletWallMHD");
...
{
    BL_PROFILE("DirichletWallMHD::solve");
    ...
    for (size_t n = 0; n < +MHDDef::PrimIdx::NUM; ++n) { cell_state[n] = p4(i, j, k, n); }
...
{
    BL_PROFILE("MHDSlipWall::MHDSlipWall");
