...
bool MHDHLLE::registered = GetMHDRiemannSolverFactory().Register(MHDHLLE::tag, MHDRiemannSolverBuilder<MHDHLLE>);
                    ...
                    Real* shk)
{
    ...
    dU[+MHDDef::ConsIdx::Eden] = (pR - pL)/(gamma-1) + 0.5*(rR*uuR+BBR) - 0.5*(rL*uuL+BBL);
    ...
    fLU[+MHDDef::ConsIdx::Eden] = (pL/(gamma-1)+0.5*(rL*uuL+BBL)+ptL)*uL - (uL*BxL+vL*ByL+wL*BzL)*BxL;
    ...
    fRU[+MHDDef::ConsIdx::Eden] = (pR/(gamma-1)+0.5*(rR*uuR+BBR)+ptR)*uR - (uR*BxR+vR*ByR+wR*BzR)*BxR;
