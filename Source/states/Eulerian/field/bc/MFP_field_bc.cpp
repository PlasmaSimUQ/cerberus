...
{
    BL_PROFILE("FieldState::set_eb_bc");

    std::string bc_type = bc_def.get_or<std::string>("type", CollectionWall::tag);

    if (bc_type == ConductingWall::tag) {
        eb_bcs.push_back(std::unique_ptr<FieldBoundaryEB>(
          new ConductingWall(global_idx, flux_solver.get(), bc_def)));
    } else if (bc_type == ScalarPotentialWall::tag) {
        eb_bcs.push_back(
          std::unique_ptr<FieldBoundaryEB>(new ScalarPotentialWall(global_idx, bc_def)));
    } else if (bc_type == SurfaceChargeWall::tag) {
        eb_bcs.push_back(
          std::unique_ptr<FieldBoundaryEB>(new SurfaceChargeWall(global_idx, bc_def)));
        ...
        eb_bcs.push_back(
          std::unique_ptr<FieldBoundaryEB>(new DefinedWall(global_idx, flux_solver.get(), bc_def)));
    } else {
...
{
    BL_PROFILE("DefinedWall::DefinedWall");
...
{
    BL_PROFILE("DefinedWall::solve");

    const Array4<const Real>& p4 = all_prim[state_idx];

    // grab the values we need
    for (size_t n = 0; n < +FieldDef::ConsIdx::NUM; ++n) { cell_state[n] = p4(i, j, k, n); }
...
{
    BL_PROFILE("ConductingWall::ConductingWall");
...
void ConductingWall::solve(Array<Array<Real, 3>, 3>& wall_coord,
                           ...
                           Array<Vector<Real>, AMREX_SPACEDIM>& F)
{
    BL_PROFILE("ConductingWall::solve");
    ...
    if (B1_defined) wall_state[+FieldDef::ConsIdx::By] = wall_B1;
    if (B2_defined) wall_state[+FieldDef::ConsIdx::Bz] = wall_B2;
    ...
    if (D_defined) wall_state[+FieldDef::ConsIdx::Dx] = wall_D;
...
{
    BL_PROFILE("ScalarPotentialWall::ScalarPotentialWall");
...
Vector<Vector<Real>>
  ScalarPotentialWall::get_wall_state(const Array<Real, AMREX_SPACEDIM> wall_centre,
                                      const Array<Array<Real, 3>, 3>& wall_coord,
                                      const Real t) const
{
    BL_PROFILE("ScalarPotentialWall::get_wall_state");
...
{
    BL_PROFILE("SurfaceChargeWall::SurfaceChargeWall");
...
Vector<Vector<Real>>
  SurfaceChargeWall::get_wall_state(const Array<Real, AMREX_SPACEDIM> wall_centre,
                                    const Array<Array<Real, 3>, 3>& wall_coord,
                                    const Real t) const
{
    BL_PROFILE("SurfaceChargeWall::get_wall_state");
...
{
    BL_PROFILE("SurfaceCurrentWall::SurfaceCurrentWall");
...
Vector<Vector<Real>>
  SurfaceCurrentWall::get_wall_state(const Array<Real, AMREX_SPACEDIM> wall_centre,
                                     const Array<Array<Real, 3>, 3>& wall_coord,
                                     const Real t) const
{
    BL_PROFILE("SurfaceCurrentWall::get_wall_state");
...
{
    BL_PROFILE("VectorPotentialWall::VectorPotentialWall");
...
Vector<Vector<Real>>
  VectorPotentialWall::get_wall_state(const Array<Real, AMREX_SPACEDIM> wall_centre,
                                      const Array<Array<Real, 3>, 3>& wall_coord,
                                      const Real t) const
{
    BL_PROFILE("VectorPotentialWall::get_wall_state");

    std::map<std::string, Real> data;

    // only load data if the boundary conditions are functions
    if (has_function) {
        AMREX_D_TERM(data["x"] = wall_centre[0];, data["y"] = wall_centre[1];
                     , data["z"] = wall_centre[2];)
...
{
    BL_PROFILE("CollectionWall::CollectionWall");
    ...
    for (const auto& type : types) {
        ...
        if (bc_type == ScalarPotentialWall::tag) {
            ...
            Abort("Requested EB bc of type '" + bc_type +
                  "' which is not compatible with 'collection'");
        }
    }
}

Vector<Vector<Real>> CollectionWall::get_wall_state(const Array<Real, AMREX_SPACEDIM> wall_centre,
                                                    const Array<Array<Real, 3>, 3>& wall_coord,
                                                    const Real t) const
{
    BL_PROFILE("CollectionWall::get_wall_state");
...
{
    BL_PROFILE("CollectionWall::time_varying");

    for (const auto& bc : bcs) {
        if (bc->time_varying()) { return true; }
    }
    return false;
}
