...
Array<int, +FieldDef::VectorIdx::Cons> FieldState::vector_idx = {+FieldDef::FieldDef::ConsIdx::Bx,
                                                                 +FieldDef::FieldDef::ConsIdx::Dx};
...
{
    BL_PROFILE("FieldState::FieldState");
...
Real FieldState::get_allowed_time_step(MFP* mfp) const
{
    ...
    for (int i = 1; i < AMREX_SPACEDIM; ++i) { dt = std::min(dt, dx[i] / fastest_speed); }
...
{
    BL_PROFILE("FieldState::get_plot_output_names");
...
{
    BL_PROFILE("FieldState::get_plot_output");
...
#ifdef AMREX_USE_EB
                if (vf4(i, j, k) == 0.0) {
                    for (const std::string& s : updated) { out4[s](i, j, k) = 0.0; }
                    continue;
                }
#endif
...
void FieldState::set_udf()
{
    ...
    if ((!value.valid() || value.empty()) && ParallelDescriptor::IOProcessor())
        Warning("WARNING: State '" + name +
                "' does not have 'value' defined for initial conditions, using defaults");
...
{
    BL_PROFILE("FieldState::set_flux");
...
{
    BL_PROFILE("FieldState::set_refinement");
    ...
    for (int ax = 0; ax < AMREX_SPACEDIM; ++ax) {
        for (int lh = 0; lh < 2; ++lh) {
            ...
            for (const auto& bc : bc2index) {
                ...
                std::string side_bc =
                  state_def["bc"][dir_name[ax]][side_name[lh]][bc.first].get_or<std::string>(
                    "outflow");
                ...
                for (const auto& var : bc.second) {
                    ...
                    const sol::object bcv =
                      state_def["bc"][dir_name[ax]][side_name[lh]][var.first].get_or(sol::object());

                    Optional3D1VFunction v;

                    // special case for phi and psi (set to zero in boundary)
                    if (var.second == +FieldDef::ConsIdx::phi ||
                        var.second == +FieldDef::ConsIdx::psi) {
                        get_udf(bcv, v, 0.0);
                    } else {
                        v = get_udf(bcv);
                    }
                    ...
                    if (i_side_bc == PhysBCType::inflow && !v.is_valid()) {
                        Abort("Setting '" + bc.first +
                              " = inflow' requires all primitive variables to be defined, '" +
                              var.first + "' is not defined");
                    }
                }
            }
...
{
    BL_PROFILE("FieldState::variable_setup");
...
{
    BL_PROFILE("FieldState::cons2prim");
...
{
    BL_PROFILE("FieldState::prim2cons");
...
{
    BL_PROFILE("FieldState::get_wall_value");
            ...
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                if (cflag.isSingleValued()) {
                    ...
                    for (size_t n = 0; n < wall_state.size(); ++n) {
                        ...
                        for (size_t n = 0; n < ws.size(); ++n) { wall4(i, j, k, n) = ws[n]; }
                    }
                }
            }
        }
...
{
    BL_PROFILE("FieldState::write_info");
