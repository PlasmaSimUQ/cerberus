...
bool TracerParticle::registered =
  GetStateFactory().Register(TracerParticle::tag, StateBuilder<TracerParticle>);

TracerParticle::TracerParticle() {}

TracerParticle::TracerParticle(const sol::table& def)
{
    BL_PROFILE("TracerParticle::TracerParticle");
...
{
    BL_PROFILE("TracerParticle::init_data");
...
{
    BL_PROFILE("TracerParticle::init");
        ...
        for (MFIter mfi = particles->MakeMFIter(level); mfi.isValid(); ++mfi) {
            ...
            for (int pi = 0; pi < initial_positions.size(); ++pi) {
                ...
                if (box.contains(pos_idx)) {
                    ...
                    AMREX_D_TERM(p.rdata(+ParticleIdxR::VX) = 0.0;
...
{
    BL_PROFILE("TracerParticle::checkpoint");
...
{
    BL_PROFILE("TracerParticle::restart");
...
{
    BL_PROFILE("TracerParticle::redistribute");
...
{
    BL_PROFILE("TracerParticle::clear");
...
{
    BL_PROFILE("TracerParticle::get_plot_output_names");
...
void TracerParticle::get_plot_output(MFP* mfp,
                                     MultiFab& plot_data,
                                     std::vector<std::string>& plot_names) const
{
    BL_PROFILE("TracerParticle::get_plot_output");
...
{
    BL_PROFILE("TracerParticle::write_info");
