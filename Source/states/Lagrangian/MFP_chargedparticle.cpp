...
Vector<std::string> ChargedParticle::particle_real_names = {
  "mass", "charge", "x_vel", "y_vel", "z_vel"};
Vector<std::string> ChargedParticle::particle_int_names = {};

std::string ChargedParticle::tag = "charged_particle";
bool ChargedParticle::registered =
  GetStateFactory().Register(ChargedParticle::tag, StateBuilder<ChargedParticle>);

ChargedParticle::ChargedParticle() {}

ChargedParticle::ChargedParticle(const sol::table& def)
{
    BL_PROFILE("ChargedParticle::ChargedParticle");
    ...
    if (!initial)
        Abort("Failed to read particle distribution definition for '" + name +
              "', must be one of " + vec2str(dfact.getKeys()));
}

void ChargedParticle::init_data(MFP* mfp, const Real time)
{
    BL_PROFILE("ChargedParticle::init_data");

    if (mfp->get_level() == 0) init(mfp);
}

// this should only be called at level 0
void ChargedParticle::init(MFP* mfp, bool make_particles)
{
    BL_PROFILE("ChargedParticle::init");
    ...
    if (make_particles) {
        ...
        case ParticleDistributionInit::Defined: {
            ...
            for (MFIter mfi = particles->MakeMFIter(level); mfi.isValid(); ++mfi) {
                ...
                CParTileType& pc =
                  particles->DefineAndReturnParticleTile(level, mfi.index(), mfi.LocalTileIndex());
                    ...
                    for (int j = lo.y; j <= hi.y; ++j) {
                        ...
                        for (int i = lo.x; i <= hi.x; ++i) {
                            ...
                            Real particle_charge =
                              (particle_mass / d_species->mass) * d_species->charge;
...
{
    BL_PROFILE("ChargedParticle::checkpoint");
...
{
    BL_PROFILE("ChargedParticle::restart");
...
{
    BL_PROFILE("ChargedParticle::redistribute");
...
{
    BL_PROFILE("ChargedParticle::clear");
...
{
    BL_PROFILE("ChargedParticle::push_particles");
    ...
    amrex::ParallelFor(n, [=] AMREX_GPU_DEVICE(int i) {
        ...
        for (int i = 0; i < 3; ++i) { p.rdata(+ParticleIdxR::VX + i) = V1[i] + E0[i]; }
...
{
    BL_PROFILE("ChargedParticle::calculate_source");
    ...
    for (int i = 0; i < np; ++i) {
        ...
        AMREX_D_TERM(coord[0] = floor((particle[i].pos(0) - dom_lo[0]) * dxi[0]);
...
{
    BL_PROFILE("ChargedParticle::set_eb_bc");
...
{
    BL_PROFILE("ChargedParticle::get_plot_output_names");
...
void ChargedParticle::get_plot_output(MFP* mfp,
                                      MultiFab& plot_data,
                                      std::vector<std::string>& plot_names) const
{
    BL_PROFILE("ChargedParticle::get_plot_output");
    ...
    for (amrex::MFIter mfi = P->MakeMFIter(level); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(n, [=] AMREX_GPU_DEVICE(int i) {
            ...
            if (get_u.first)
                plot4(iloc[0], iloc[1], iloc[2], get_u.second) += p.rdata(+ParticleIdxR::VX);
            if (get_v.first)
                plot4(iloc[0], iloc[1], iloc[2], get_v.second) += p.rdata(+ParticleIdxR::VY);
            if (get_w.first)
                plot4(iloc[0], iloc[1], iloc[2], get_w.second) += p.rdata(+ParticleIdxR::VZ);
        });
...
{
    BL_PROFILE("ChargedParticle::write_info");
