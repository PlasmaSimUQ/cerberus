...
bool Lorentz::registered =
    GetSourceTermFactory().Register(Lorentz::tag, SourceTermBuilder<Lorentz>);
...
Lorentz::Lorentz(const sol::table &def) {
  ...
  if (!GD::plasma_params_set) {
    Abort("Plasma parameters required for plasma5 source, define (>0) either "
          "skin_depth & beta or Larmor & Debye");
  }
  ...
  for (const auto &idx : offsets) {
    ...
    case +StateType::isField:
      if (found_em)
        Abort("Too many fields linked to plasma5 source");
...
int Lorentz::fun_rhs(Real x, Real y, Real z, Real t, Vector<Real> &y0,
                     Vector<Real> &ydot, Real dt) const {
  ...
  for (const auto &idx : offsets) {
    ...
    ydot[idx.solver + +HydroState::ConsIdx::Xmom] +=
        (rho * r / Larmor) * (lightspeed * Ex + v * Bz - w * By);
    ydot[idx.solver + +HydroState::ConsIdx::Ymom] +=
        (rho * r / Larmor) * (lightspeed * Ey + w * Bx - u * Bz);
    ydot[idx.solver + +HydroState::ConsIdx::Zmom] +=
        (rho * r / Larmor) * (lightspeed * Ez + u * By - v * Bx);
  }

  return 0;
}

int Lorentz::fun_jac(Real x, Real y, Real z, Real t, Vector<Real> &y0,
                     Vector<Real> &J) const {
  ...
  // get all of the field data (should only be one field associated with this
  // source
  Real ep;
  Real Bx, By, Bz;
  Real Ex, Ey, Ez;
  ...
  for (const auto &idx : offsets) {
    ...
    JJ(off + +HydroState::ConsIdx::Xmom, off + +HydroState::ConsIdx::Ymom) =
        Bz * r_dL;
    JJ(off + +HydroState::ConsIdx::Xmom, off + +HydroState::ConsIdx::Zmom) =
        -By * r_dL;

    JJ(off + +HydroState::ConsIdx::Ymom, off + +HydroState::ConsIdx::Xmom) =
        -Bz * r_dL;
    JJ(off + +HydroState::ConsIdx::Ymom, off + +HydroState::ConsIdx::Zmom) =
        Bx * r_dL;

    JJ(off + +HydroState::ConsIdx::Zmom, off + +HydroState::ConsIdx::Xmom) =
        By * r_dL;
    JJ(off + +HydroState::ConsIdx::Zmom, off + +HydroState::ConsIdx::Ymom) =
        -Bx * r_dL;
  }
...
Real Lorentz::get_max_freq(Vector<Real> &y) const {
