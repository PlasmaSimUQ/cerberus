...
Lorentz::Lorentz(const sol::table& def)
{
    ...
    if (linked_EM_idx < 0) Abort("No electromagnetic field found for plasma5 source");
}
...
int Lorentz::fun_rhs(
  Real x, Real y, Real z, Real t, Vector<Real>& y0, Vector<Real>& ydot, Real dt) const
{
    BL_PROFILE("Lorentz::fun_rhs");
    ...
    for (const auto& idx : offsets) {
        ...
        if (tp != +StateType::isField) continue;
...
{
    BL_PROFILE("Lorentz::get_max_freq");
    ...
    for (const auto& idx : offsets) {
        ...
        if (t != +StateType::isField) continue;
