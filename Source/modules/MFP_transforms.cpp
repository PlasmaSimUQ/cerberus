...
Array<Real, 3> facefrac2normal(const Array<Array<Real, 2>, AMREX_SPACEDIM>& alpha)
{
    ...
    Real apnorm =
      sqrt(AMREX_D_TERM(normal[0] * normal[0], +normal[1] * normal[1], +normal[2] * normal[2]));

    if (apnorm == 0.0) { Abort("Error: wall normal length = 0"); }
...
void expand_coord(Array<Array<Real, 3>, 3>& coord)
{
    ...
    coord[2] = {coord[0][1] * coord[1][2] - coord[0][2] * coord[1][1],
