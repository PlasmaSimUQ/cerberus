...
std::vector<double> get_numbers(std::string s)
{
    ...
    while (std::regex_search(s, m, re)) {
        for (auto x : m) { numbers.push_back(std::stod(x)); }
        s = m.suffix().str();
    }
    return numbers;
}
...
void insert_segment(Vector<Array<RealVect, 4>>& curve, const Array<RealVect, 4>& seg)
{
    if (!curve.empty()) {
        ...
        for (size_t i = 0; i < 3; ++i) {
            if (prev[i] != prev[i + 1]) { has_difference = true; }
        }

        // if it is a point, remove it!
        if (!has_difference) { curve.pop_back(); }
    }
...
float svg_angle(Real ux, Real uy, Real vx, Real vy)
{
    ...
    Real ang = std::acos(
      std::clamp(dot / len, -1.0, 1.0));  // floating point precision, slightly over values appear
...
RealVect elliptic_arc_point(RealVect c, RealVect r, float phi, float t)
{
    Real a = std::sin(phi);
    Real b = std::cos(phi);
    return RealVect(AMREX_D_DECL(c[0] + r[0] * b * std::cos(t) - r[1] * a * std::sin(t),
...
RealVect elliptic_arc_derivative(RealVect r, float phi, float t)
{
    ...
    Real ct = std::cos(t);
    return RealVect(
      AMREX_D_DECL(-r[0] * b * st - r[1] * a * ct, -r[0] * a * st + r[1] * b * ct, 0.0));
}

Vector<Array<RealVect, 4>> get_bezier_elliptical_arc(
  Real x1, Real y1, Real rx, Real ry, Real phi, Real fA, Real fS, Real x2, Real y2)
{
    ...
    //(F.6.5.6)
    Real delta =
      svg_angle((x1p - cxp) / rX, (y1p - cyp) / rY, (-x1p - cxp) / rX, (-y1p - cyp) / rY);
    delta = std::fmod(delta, pi * 2.0);
    if (!fS) delta -= 2 * pi;
    //    int sign = (end_angle < start_angle) ? -1 : 1;

    // we now have the centre of the ellipse (cx, cy) as well as the angles of each of the end
    // points
...
ReadSVG::ReadSVG(const std::string& path, const Real dx)
{
    ...
    std::vector<double> viewBox =
      get_numbers(std::string(root_node->first_attribute("viewBox")->value()));
    ...
    for (auto& shape : paths) {
        if (shape.first.find("origin") != std::string::npos) { doc_origin = shape.second[0][0]; }
    }
...
std::map<std::string, Vector<Array<RealVect, 4>>>
  ReadSVG::get_elements(rxml::xml_node<>* node, const std::string& parent_name)
{
    BL_PROFILE("ReadSVG::get_elements");

    std::map<std::string, Vector<Array<RealVect, 4>>> curves;

    // we're probably going to use inkscape so test for inkscape labels before falling back to the
    // generic id
    std::string id;
    ...
    if ((tag == "g") || (tag == "svg")) {
        ...
        for (rxml::xml_node<>* sub_node = node->first_node(); sub_node != NULL;
             sub_node = sub_node->next_sibling()) {
            std::map<std::string, Vector<Array<RealVect, 4>>> sub_curves =
              get_elements(sub_node, level_name);
        ...
        while (i < words.size()) {
            ...
            if (w == "m") {
                ...
                for (const auto& seg : get_bezier_elliptical_arc(p0[0],
                                                                 p0[1],
                                                                 num[0],
                                                                 num[1],
                                                                 num[2],
            ...
            i += 1;
            //            std::cout << curve.back()[0] << ", " << curve.back()[1] << ", " <<
            //            curve.back()[2] << ", "<< curve.back()[3] << std::endl;
        }
    ...
    if (attr) {
        ...
        if (found != std::string::npos) {
            ...
            if (numbers.size() == 2) { d = numbers[1]; }
        }

        // skewX
        found = transform.find("skewX");
        if (found != std::string::npos) { c = sin(numbers[0]); }

        // skewY
        found = transform.find("skewY");
        if (found != std::string::npos) { b = sin(numbers[0]); }
...
Real ReadSVG::query(AMREX_D_DECL(Real x, Real y, Real z), std::string layer)
{
    ...
    if (polysplines.find(layer) == polysplines.end()) {
        ...
        for (auto& ps : polysplines) { names.push_back(ps.first); }
        std::stringstream msg;
        msg << "ReadSVG requested layer '" << layer
            << "' which does not exist. Try :" << vec2str(names);
        Abort(msg.str());
    }
...
{
    BL_PROFILE("ReadSVG::register_with_lua");

    lua.new_usertype<ReadSVG>("ReadSVG",
                              sol::constructors<ReadSVG(const std::string&, const Real)>(),
                              "query",
                              &ReadSVG::query);
}
