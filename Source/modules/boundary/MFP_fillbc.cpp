...
{
    BL_PROFILE("StateBC::add_function");

    functions[name] = fun;

    if (fun.has_func()) { has_function = true; }
    if (fun.is_valid()) { is_valid = true; }
}

void BoundaryState::set(int d, const std::string name, int s, const Optional3D1VFunction& f)
{
    BL_PROFILE("BoundaryState::set");
...
{
    BL_PROFILE("BoundaryState::set_lo");
...
{
    BL_PROFILE("BoundaryState::set_hi");
...
const StateBC& BoundaryState::get_lo(int dir) const { return data[0][dir]; }

const StateBC& BoundaryState::get_hi(int dir) const { return data[1][dir]; }

void BoundaryState::post_init()
{
    BL_PROFILE("BoundaryState::post_init");
    ...
    for (int d = 0; d < AMREX_SPACEDIM; ++d) {
        for (int i = 0; i < 2; ++i) {
            ...
            if (where_is_inflow.data()[i * AMREX_SPACEDIM + d] == BCType::ext_dir) {
                ...
                for (std::map<std::string, Optional3D1VFunction>::const_iterator it = m.begin();
                     it != m.end();
                     ++it) {
...
FillBC* FillBC::clone() const { return new FillBC(*this); }

bool FillBC::hasFabVersion() const noexcept
{
    BL_PROFILE("FillBC::hasFabVersion");

    return true;
}

void FillBC::operator()(Box const& bx,
                        FArrayBox& dest,
                        const int dcomp,
                        const int numcomp,
                        Geometry const& geom,
...
NullFillBC* NullFillBC::clone() const { return new NullFillBC(*this); }

bool NullFillBC::hasFabVersion() const noexcept
{
    BL_PROFILE("NullFillBC::hasFabVersion");

    return true;
}

void NullFillBC::operator()(Box const& bx,
                            FArrayBox& dest,
                            const int dcomp,
                            const int numcomp,
                            Geometry const& geom,
                            ...
                            const int orig_comp) const
{
    BL_PROFILE("NullFillBC::operator()");
