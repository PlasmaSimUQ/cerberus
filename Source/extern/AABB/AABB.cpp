...
double point2triangleDistance(const double* pt, const double* tri_0, const double* tri_1, const double* tri_2)
{
...
int point2triangleSide(const double* pt, const double* tri_0, const double* tri_1, const double* tri_2)
{
...
std::vector<double> AABB::closestPoint(const std::vector<double>& p) const
{
    ...
    for( int i = 0; i < dim; i++ ) {
        double v = p[i];
        if( v < lowerBound[i] ) v = lowerBound[i]; // v = max( v, b.min[i] )
        if( v > upperBound[i] ) v = upperBound[i]; // v = min( v, b.max[i] )
        q[i] = v;
    }
...
void Tree::init(unsigned int dimension_,
...
double Tree::signedDistance(const std::vector<double>& xyz, const std::vector<std::array<double,9>>& tris)
{
    ...
    while (stack.size() > 0)
    {
        ...
        if (sphere.overlaps(nodeAABB, touchIsOverlap))
        {
            ...
            if (nodes[node].isLeaf())
            {
                ...
                double rs = point2triangleDistance(sphere.centre.data(),&facet[0], &facet[3], &facet[6]);

                // catch any floating point errors
                // (moved here as optimization in point2triangleDistance function just returns 0.0 !!??!!)
                rs = std::max(rs, 0.0);
    ...
    int side = point2triangleSide(sphere.centre.data(),
                                  &tris[min_particle][0],
            &tris[min_particle][3],
            &tris[min_particle][6]);
