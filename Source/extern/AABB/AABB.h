...
namespace aabb
{
    ...
    public:
        ...
        //! Calculate the closest point on the AABB to the provided point
        /*! \param xyz
                Vector defining query point

            \return
                Vector defining closest point to query point.
         */
        std::vector<double> closestPoint(const std::vector<double>&) const;
        ...
        /*! \param xyz
            ...
            \return
                bool true if inside;
         */
        bool isInside(const std::vector<double>& p) const;

        //! Calculate the distance to the closest point on the AABB to the provided point
        /*! \param xyz
                Vector defining query point

            \return
                Distance from closest point to query point.
         */
        double closestDistance(const std::vector<double>& xyz) const;
    ...
    /*! \brief The sphere object

        Defined by a centre point and radius
     */
    ...
    public:
        ...
        //! Constructor.
        /*! \param centre
                Vector defining centre of sphere point.

            \param centre
                Radius of sphere.

         */
        Sphere(const std::vector<double>&, double);

        //! Check if this sphere overlaps the provided AABB
        /*! \param aabb
                AABB to check against

            \param touchIsOverlap
                Flag to say touching is considered an overlap.

         */
        bool overlaps(const AABB&, bool) const;
    ...
    public:
         ...
         */
        void init(unsigned int dimension_= 3, double skinThickness_ = 0.05,
        ...
        //! Query the tree to find the closest point on any of the provided triangles to a query point.
        /*! \param xyz
                The query point.

            \param tris
                Vector of all triangles reference by the tree.

            \return particles
                Signed distance to closest point.
         */
        double signedDistance(const std::vector<double>&, const std::vector<std::array<double, 9> > &tris);

        //! Query to find if a point is inside the root node of the tree.
        /*! \param xyz
                The query point.
            ...
            \return bool
                True if inside.
         */
        bool isInside(const std::vector<double>& xyz) const;
