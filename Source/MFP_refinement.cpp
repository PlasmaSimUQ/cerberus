...
void MFP::errorEst(TagBoxArray& tags, int, int, Real time, int, int)
{
    ...
    for (const auto& istate : states) { istate->get_refinement_tags(this, tags); }
    ...
    #pragma omp parallel
        ...
        for (MFIter mfi(cost); mfi.isValid(); ++mfi) {
            ...
            for (BoxIterator bi(bx); bi.ok(); ++bi) {
                ...
                if (only_refine_in_box) set_type = TagBox::CLEAR;
                ...
                for (int bidx = 0; bidx < refine_boxes.size(); ++bidx) {
                    ...
                    if (rbx.contains(pos)) {
                        if (refine_box_type[bidx] == RefineBoxType::OnlyRefine) {
                            ...
                            set_type = std::max((int)TagBox::CLEAR,
                                                set_type);  // clear, but only if we haven't already
                ...
                if (set_type > -1) fab(cell) = set_type;
            }
        }
    }
}
